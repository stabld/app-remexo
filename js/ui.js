    ...
    try {
        ...
        if (!isMarket) {
            ...
            return '<div id="market-card-' + req.id + '" class="market-item bg-white dark:bg-[#0f172a] rounded-3xl border border-slate-200 dark:border-slate-800 p-6 hover:border-remexo-500/50 hover:shadow-xl transition-all duration-300 cursor-pointer fade-up overflow-hidden relative group" data-kat="' + reqCat + '" style="animation-delay:' + (i*60) + 'ms">' +
                ...
                marketDetailsHtml +
                '<div class="flex gap-3 mt-6 pt-5 border-t border-slate-100 dark:border-slate-800"><button onclick="window.openOfferModal(\'' + req.id + '\')" class="flex-1 bg-remexo-500 hover:bg-remexo-600 text-white py-3.5 rounded-xl font-bold text-sm transition shadow-md hover:scale-[1.02]">Podat nabídku zákazníkovi</button><button onclick="window.showOnMap(\'' + req.id + '\')" title="Zobrazit na mapě" class="px-4 h-12 border-2 border-slate-200 dark:border-slate-700 rounded-xl flex items-center justify-center gap-2 text-slate-500 dark:text-slate-400 hover:text-remexo-500 hover:border-remexo-500 hover:bg-remexo-50 dark:hover:bg-remexo-500/10 transition-colors font-bold text-sm"><i class="fa-solid fa-map-location-dot"></i><span class="hidden sm:inline">Na mapě</span></button><button class="w-12 h-12 border-2 border-slate-200 dark:border-slate-700 rounded-xl flex items-center justify-center text-slate-400 hover:text-remexo-500 hover:border-remexo-500 hover:bg-remexo-50 dark:hover:bg-remexo-500/10 transition-colors shrink-0"><i class="fa-regular fa-bookmark"></i></button></div>' +
                '</div></div></div></div>';
        }
