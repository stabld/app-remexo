        ...
        if (mainDesc.includes("---")) {
            ...
            if (pts.length > 1) {
                ...
                if (detailItems.length > 0) {
                    detailsHtml = '<div class="req-detaily mt-5 pt-5 border-t border-slate-100 dark:border-slate-700/50"><p class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">Doplňující informace</p><div class="flex flex-wrap gap-2">' +
            ...
            return '<div class="req-card relative bg-white dark:bg-[#0f172a] p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all duration-300 group fade-up overflow-hidden">' +
                ...
                (req.status==='active'
                    ? (req.dokonceniNavrzeno
                        ...
                        : '<button onclick="window.openRatingModal(' + i + ',' + (req.sbId||'null') + ')" class="px-6 py-3.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"><i class="fa-solid fa-check mr-2"></i>Označit hotovo</button>')
                    : '') +
                '</div></div></div>';
        } else {
...
window.refreshDashboard = function() {
    ...
    if(dl&&window.STATE.requests.length>0){
        dl.innerHTML=window.STATE.requests.slice(0,3).map(r=>'<div class="flex items-center gap-3 py-3.5 lg:py-4 border-b border-slate-100 dark:border-slate-800 last:border-0"><div class="w-10 h-10 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 rounded-xl flex items-center justify-center text-sm shrink-0"><i class="fa-solid fa-clipboard-list"></i></div><div class="flex-1 min-w-0"><p class="font-extrabold text-sm dark:text-white truncate">' + window.escapeHtml(r.title || "Poptávka") + '</p><p class="text-xs text-slate-500 mt-0.5">' + window.escapeHtml(r.kat || "") + ' • ' + window.escapeHtml(r.time || "") + '</p></div><span class="status-badge ' + (r.status==="done"?"status-done":r.status==="active"?"status-active":"status-waiting") + ' shrink-0">' + (r.status==="done"?"Hotovo":r.status==="active"?"Probíhá":"Čeká") + '</span></div>').join("");
    } else if(dl){
        dl.innerHTML='<p class="text-slate-400 text-center py-5 lg:py-8 text-sm lg:text-base">Zatím žádné poptávky. <button onclick="window.goTab(\'new\',\'Nová poptávka\')" class="text-remexo-500 font-bold hover:underline">Vytvořit první →</button></p>';
    }
};
