...
window.refreshDashboard = function() {
    ...
    if(dl&&window.STATE.requests.length>0){
        dl.innerHTML=window.STATE.requests.slice(0,3).map(r=>'<div class="flex items-center gap-3 py-2.5 lg:py-4 border-b border-slate-100 dark:border-slate-800 last:border-0"><div class="w-9 h-9 lg:w-10 lg:h-10 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 rounded-xl flex items-center justify-center text-sm shrink-0"><i class="fa-solid fa-clipboard-list"></i></div><div class="flex-1 min-w-0"><p class="font-extrabold text-sm dark:text-white truncate">' + r.title + '</p><p class="text-xs text-slate-500 mt-0.5">' + r.kat + ' • ' + r.time + '</p></div><span class="status-badge ' + (r.status==="done"?"status-done":r.status==="active"?"status-active":"status-waiting") + ' shrink-0">' + (r.status==="done"?"Hotovo":r.status==="active"?"Probíhá":"Čeká") + '</span></div>').join("");
    } else if(dl){
